    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            // Show Delete button
            echo "<button onclick='confirmDelete({$row['id']})' class='text-red-500'>Delete</button>";

            // Show Join button if the meeting time has passed or is currently ongoing
            if ($current_datetime >= $meeting_datetime) {
                echo "<br><button onclick=\"window.open('{$row['zoom_link']}', '_blank')\" class='text-green-500 mt-2'>Join</button>";
            }
